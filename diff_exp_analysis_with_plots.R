...
txi$length
gene_lengths <- data.frame(row.names = rownames(counts), rowMeans(txi$length)) 
gene_lengths
...
counts_total <- read.csv("~/bioinformatics/counts.csv")

counts_lengths <- merge(counts_total, gene_lengths, by.x = "name", by.y = "row.names", all.x = TRUE)
counts_lengths$rowMeans.txi.length.
hist(counts_lengths, aes(x = rowMeans.txi.length., y = total))



bayseq <- read.csv("~/bioinformatics/bayseq_res.csv")
merged_df <- merge(bayseq, gene_lengths, by.x = "name", by.y = "row.names", all.x = TRUE)
hist(merged_df$rowMeans.txi.length., main="BaySeq", xlim=c(0,20000),
...
edger <- read.csv("~/bioinformatics/DE_results_with_length/edger_lengths.csv")
noiseq <- read.csv("~/bioinformatics/noi_res.csv")
deseq <- read.csv("~/bioinformatics/deseq_res.csv")
samr <- read.csv("~/bioinformatics/DE_results_with_length/samr_lengths.csv")
bay <- read.csv("~/bioinformatics/bayseq_res.csv")
...
merged_df <- merge(bay, gene_lengths, by.x = "name", by.y = "row.names", all.x = TRUE)
bayes_de <- subset(merged_df, x < 0.05 )

length(limma_de$rowMeans.txi.length.)
length(edger_de$rowMeans.txi.length.)
length(noiseq_de$rowMeans.txi.length.)
length(deseq_de$rowMeans.txi.length.)
length(samr_de$rowMeans.txi.length.)



# Set up a grid layout with 2 rows and 3 columns
par(mfrow = c(2, 3))  # Adjust rows and columns as needed

# LIMMA
hist(merged_df$rowMeans.txi.length., main="limma", xlim=c(0,20000),
     col = rgb(1, 0, 0, 0.5), breaks = 100, xlab = "Gene Length", prob=TRUE)
hist(limma_de$rowMeans.txi.length., breaks = 100, 
     col = rgb(0, 0, 1, 0.5), add = TRUE, prob=TRUE)
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# EdgeR
hist(merged_df$rowMeans.txi.length., main="EdgeR", xlim=c(0,20000),
     col = rgb(1, 0, 0, 0.5), breaks = 100, xlab = "Gene Length", prob=TRUE)
hist(edger_de$rowMeans.txi.length., breaks = 100, 
     col = rgb(0, 0, 1, 0.5), add = TRUE, prob=TRUE)
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# NOISeq
hist(merged_df$rowMeans.txi.length., main="NOISeq", xlim=c(0,20000),
     col = rgb(1, 0, 0, 0.5), breaks = 100, xlab = "Gene Length", prob=TRUE)
hist(noiseq_de$rowMeans.txi.length., breaks = 100, 
     col = rgb(0, 0, 1, 0.5), add = TRUE, prob=TRUE)
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# DESeq2
hist(merged_df$rowMeans.txi.length., main="DESeq2", xlim=c(0,20000),
     col = rgb(1, 0, 0, 0.5), breaks = 100, xlab = "Gene Length", prob=TRUE)
hist(deseq_de$rowMeans.txi.length., breaks = 100, 
     col = rgb(0, 0, 1, 0.5), add = TRUE, prob=TRUE)
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))

# SAMR
hist(merged_df$rowMeans.txi.length., main="SAMR", xlim=c(0,20000),
     col = rgb(1, 0, 0, 0.5), breaks = 100, xlab = "Gene Length", prob=TRUE)
hist(samr_de$rowMeans.txi.length., breaks = 100, 
     col = rgb(0, 0, 1, 0.5), add = TRUE, prob=TRUE)
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))
...
hist(bayes_de$rowMeans.txi.length., breaks = 100,prob=TRUE, 
     col = rgb(0, 0, 1, 0.5), add = TRUE )
legend("topright", legend = c("All genes", "Differentially expressed"), 
       fill = c(rgb(1, 0, 0, 0.5), rgb(0, 0, 1, 0.5)))
...
ks.test(x=gene_lengths[1],y=noiseq_de$)
