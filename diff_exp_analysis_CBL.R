...
# Import aligned data
files <- list.files(path = "quants", pattern = ".sf", full.names = TRUE, 
                    recursive = TRUE)
sample_names <- gsub(".*/(.*)\\.quant\\.sf", "\\1", files)

gtf_file <- "Drosophila_melanogaster.BDGP6.46.113.gtf"  # Update with the correct path
gtf_data <- rtracklayer::import(gtf_file)

# Extract transcript-to-gene mapping from GTF
tx2gene <- data.frame(
  transcript_id = mcols(gtf_data)$transcript_id,  # Transcript IDs
  gene_id = mcols(gtf_data)$gene_id              # Gene IDs
)
...
meta <- data.frame(
  ...
  Condition = c("Control", "Control", "Control","Control","Starvation", "Starvation","Starvation","Starvation"), # Add Starvation also as condition
  path = files
)
